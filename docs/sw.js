...
self.addEventListener('fetch', e => {
    ...
    // Match __box__ within the SW scope (works for subpaths like /boxvm/)
    const scope = self.registration ? self.registration.scope : '/';
    const boxPrefix = new URL('__box__/', scope).pathname;
    if (!url.pathname.startsWith(boxPrefix)) return;
    let path = url.pathname.replace(boxPrefix, '');
    if (!vfs.has(path)) {
